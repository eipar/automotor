...
#define CS_PIN 30 // Pin de CS
#define AWAKE_PIN 31 // Pin de AWAKEs

#define TXD0_BIT_MASK  (1 << TXD0_PIN) //TXD0
#define RXD0_BIT_MASK  (1 << RXD0_PIN) //RXD0
#define LED_BIT_MASK (1 << LED_PIN) // LED
#define CS_BIT_MASK (1 << CS_PIN) // CS
#define AWAKE_BIT_MASK (1 << AWAKE_PIN) // AWAKE
...
int main(void)
{
	...
	PinCfg_UART0.Funcnum = 1;     // De PINSEL0 , función 01b, para TXD0
	PinCfg_UART0.OpenDrain = 0;   //No salida open drain
	PinCfg_UART0.Pinmode = 0;     //Sin pull up
	PinCfg_UART0.Pinnum = 3;      //Pin 0 para RXD0
	PinCfg_UART0.Portnum = 0;     //Port 0
	...
	GPIO_ClearValue(LED_PORT, LED_BIT_MASK);
	...
	while(1){
		...
		if(LIN_Receive(&frame_rx)){
			for(i=0;i<10000;i++);
			if(frame_rx.DataBytes[0] == PRENDER) GPIO_SetValue(LED_PORT, LED_BIT_MASK);
			else GPIO_ClearValue(LED_PORT, LED_BIT_MASK);
		}
		else {
			for(i=0;i<10000;i++);
		}
...
void Set_UART_0 (UART_FITO_LEVEL_Type LV)
{
	...
	// Por default: Baudrate = 4800bps, 8 data bit, 1 Stop bit, None parity
...
void LIN_HardInit (void) {
	...
	// AWAKE
	GPIO_SetDir(AWAKE_PORT, AWAKE_BIT_MASK , SALIDA);  	//Configuro el pin como salida
	GPIO_ClearValue(AWAKE_PORT, AWAKE_BIT_MASK);
	//Delay
	for(i = 0; i<100 ; i++);

	// CS
	GPIO_SetDir(CS_PORT, CS_BIT_MASK , SALIDA);  	//Configuro el pin como salida
...
uint8_t LIN_Send(LIN_frame *frame){
	...
	PINSEL_ConfigPin(&PinCfg_UART0); //cargo el registro PINSEL con los valores anteriores
